...
calcPerc <- function(num, denom) {
  ...
  # den = denominator, usually in the form of Field Goals attempted.
...
# Get the field goal percentage fo qualifying shooters for each zone-range combination.
allZones <- NBAFull %>%
...
ggplot(zoneNames, aes(x = ZONE_NAME, y = FGPerc, fill = SHOT_TYPE)) +
  geom_boxplot() +  # Add notches and remove outliers
  scale_fill_manual(values = c('2PT Field Goal' = '#ADD8EF', '3PT Field Goal' = '#FF999D')) +
  theme_minimal() +
  theme(legend.background = element_rect(fill = 'white', color = 'black', linewidth = 0.4, linetype = 'solid'),
        plot.background = element_rect(fill = 'white', color = 'white')) +
...
```
The scatterplot features a basketball court and points representing the median locations of shots taken in specific ranges and orientations. The percentages are the field goal percentages for each range/orientation combination.
...
Now, let’s run a simulation that compares the highest expected value field goals for both 2- and 3-point field goals.
